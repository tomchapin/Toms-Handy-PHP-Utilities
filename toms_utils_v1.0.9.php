...
// Additional Notes: Based on code found at http://detectmobilebrowsers.com/
// Parameters: none
...
// Function used to trim a string to a certain length, according to the nearest word boundary, and add a "..." to the end.
// Parameters:
// $string (required string) - the string to be analyzed and trimmed if need be
// $trim_length (optional int) - maximum number of character to trim string down to
// $delimiter (optional string) - the string to add onto the end if trimmed
// Returns: (string)
function neat_trim($string, $trim_length=200, $delimiter='...') {
	$string = trim($string);
	if (strlen($string) > $trim_length) {
		preg_match('/(.{' . $trim_length . '}.*?)\b/', $string, $matches);
		$string = trim($matches[1]) . $delimiter;
